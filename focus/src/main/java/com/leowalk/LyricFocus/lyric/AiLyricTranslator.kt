...
class AiLyricTranslator(context: Context) {
    ...
    suspend fun translateIfNeeded(lyricInfo: LyricInfo, title: String, artist: String): LyricInfo {
        ...
        if (lyricInfo.lines.any { !it.translation.isNullOrBlank() }) return lyricInfo

        val cacheKey = buildCacheKey(title, artist, lyricInfo)
    ...
    private suspend fun requestTranslation(
        ...
        val baseUrl = FocusPreferences.getAiApiBaseUrl(appContext).trimEnd('/')
        ...
        val request = Request.Builder()
            ...
            .header("User-Agent", "LyricFocus/1.6.0")
            .build()
    ...
    private fun buildCacheKey(title: String, artist: String, lyricInfo: LyricInfo): String {
        return listOf(
            title,
            artist,
