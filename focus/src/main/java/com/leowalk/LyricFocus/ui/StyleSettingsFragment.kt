...
class StyleSettingsFragment : Fragment(R.layout.activity_style_settings) {
    ...
    private val styleChangeHandler = android.os.Handler(android.os.Looper.getMainLooper())
    private val styleChangeDebounceRunnable = Runnable { notifyStyleChangedInternal() }
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        styleChangeHandler.removeCallbacks(styleChangeDebounceRunnable)
    }
    ...
    private fun bindUiFromPreferences() {
        ...
        when (FocusPreferences.getFocusBackground(requireContext())) {
            ...
            FocusPreferences.BACKGROUND_ALBUM -> backgroundGroup.check(R.id.background_album)
    ...
    private fun updateDynamicColorUi() {
        ...
        val bgAlbum = FocusPreferences.getFocusBackground(requireContext()) == FocusPreferences.BACKGROUND_ALBUM
        extractedOpacitySection.visibility = if (anyExtraction || bgCustom || bgAlbum) View.VISIBLE else View.GONE
    ...
    private fun setupListeners() {
        ...
        sliderMultiLineHeight.addOnChangeListener { _, value, fromUser ->
            tvMultiLineHeightLabel.text = "${value.toInt()} dp"
        }
        sliderMultiLineHeight.addOnSliderTouchListener(object : Slider.OnSliderTouchListener {
            override fun onStartTrackingTouch(slider: Slider) = Unit
            override fun onStopTrackingTouch(slider: Slider) {
                if (!FocusPreferences.isMultiLineLyrics(requireContext()) ||
                    !sliderMultiLineHeight.isEnabled
                ) {
                    return
                }
        ...
        val bgListener = MaterialButtonToggleGroup.OnButtonCheckedListener { group, checkedId, isChecked ->
            ...
            val background = when (checkedId) {
                ...
                R.id.background_album -> FocusPreferences.BACKGROUND_ALBUM
    ...
    private fun notifyStyleChanged() {
        // 防抖：滑块/开关连续触发时合并为最后一次，避免频繁广播导致 SystemUI 重建焦点通知掉帧
        styleChangeHandler.removeCallbacks(styleChangeDebounceRunnable)
        styleChangeHandler.postDelayed(styleChangeDebounceRunnable, 150)
    }

    private fun notifyStyleChangedInternal() {
        if (!isAdded) return
