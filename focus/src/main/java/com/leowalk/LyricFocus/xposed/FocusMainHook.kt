...
class FocusMainHook : XposedModule() {
    ...
    override fun onPackageReady(param: XposedModuleInterface.PackageReadyParam) {
        try {
            ...
            // 焦点通知歌词开关：仅在明确关闭时跳过 hook。
            // 查询失败（null）按开启处理，保持与 1.8.9「始终注入」等价，避免 OS3
            // 冷启动误判导致 canShowFocus bypass 丢失、焦点通知退化为普通通知。
            val focusEnabled = readMode("focus_mode")
            if (focusEnabled == false) {
                // 早期 Application 可能尚未就绪，ContentProvider 查询失败会误判为关闭。
                // 延迟重试，等 app 就绪后重新判定；若仍明确为 false 再跳过。
                log(android.util.Log.INFO, TAG, "focus lyric reported disabled, retry after app ready")
    ...
    /**
     * 读取模块开关状态：
     * 1. 优先系统属性（system_app 进程可读，跨进程最可靠）；
     * 2. 再试本应用 ContentProvider；
     * 3. 失败时直接解析 prefs XML 文件；
     * 4. 再失败回退 getRemotePreferences（仅当键存在时返回值）；
     * 5. 全部失败或键缺失返回 null（由调用方决定：null ≠ 关闭）。
     */
    private fun readMode(method: String): Boolean? {
        ...
        val defaultIfPresent = method != "aodchange_mode"
        ...
        return try {
            ...
            if (remote.contains(prefName)) {
                // 键存在才取值；缺键不得用 false 默认，否则 OS3 会误 skip 全部 hook
                val value = remote.getBoolean(prefName, defaultIfPresent)
