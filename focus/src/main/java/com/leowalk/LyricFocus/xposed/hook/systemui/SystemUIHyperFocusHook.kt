...
class SystemUIHyperFocusHook : BaseHook() {
    ...
    private fun hookSystemUIContext(classLoader: ClassLoader, module: XposedModule) {
        try {
            // HyperOS3: SystemUIApplication.onCreate（非 HyperOS4 的 SystemUIApplicationImpl）
    ...
    private fun hookFocusPermissionBypass(classLoader: ClassLoader, module: XposedModule) {
        // HyperOS3: boolean canShowFocus / canCustomFocus（miui.systemui 包名）
        ...
        // 插件 ClassLoader 上再补一次 boolean bypass（部分机型从 MIUISystemUIPlugin 加载）
        hookPluginClassLoader(classLoader, module)
    }

    /** 焦点通知插件类加载器：NotificationSettingsManager 可能从插件加载 */
    ...
    /** 插件类加载器中的白名单方法（OS3：仅 boolean canShowFocus / canCustomFocus） */
    ...
    private fun refreshSettings() {
        // SystemUI 进程（uid=1000）在 HyperOS3 上无法可靠读取应用私有 prefs，
        // 样式靠广播同步；此处只用 ContentProvider 刷新焦点总开关，
        // 禁止 reloadFromDisk，避免把广播样式覆盖回默认值。
