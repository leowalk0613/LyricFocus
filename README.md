...
| 项目 | 要求 |
|------|------|
| 系统 | **小米 HyperOS**（验证环境：HyperOS 3.0.x，如 `3.0.302.0.WNCCNXM`） |
...
- **通知合并优化**：通知栏歌词与前台服务通知合并为一个通知，减少通知数量
- **歌词显示优化**：在歌词加载前或无歌词时，优先显示歌名和歌手信息，提升用户体验
- **界面逻辑完善**：当还没到第一句歌词时，显示歌名+歌手而非占位符
- **无歌词状态改进**：无歌词时第一行显示歌名，第二行显示歌手
